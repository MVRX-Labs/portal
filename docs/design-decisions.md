...
**Decision:** Twitter/X functionality mirrors LinkedIn exactly — separate DB tables (`twitter_profiles`, `twitter_posts`, etc.), separate sync tasks (`twitter-sync-scheduler`, `twitter-sync-profile`), separate analytics and engagement flows — rather than being bolted onto the LinkedIn schema.

**Why:** LinkedIn and Twitter have fundamentally different data shapes (tweet IDs vs Apify post IDs; retweets/quote-tweets vs reposts; views/bookmarks unique to Twitter). Keeping them separate avoids nullable columns, union-type confusion, and query complexity on heavily-used tables. The blast radius of a Twitter outage is contained. Feature flags (`analyticsEnabled`, `outboundEnabled`, `inboundEnabled`) are replicated verbatim on `twitter_profiles`. Twitter sync runs every 4 hours (vs LinkedIn's 2h) due to stricter rate limits. See `docs/plans/completed/twitter-parallel.md` for the full plan.

---

## Alpha Feed: ICP-Level Content Intelligence

**Decision:** Each ICP definition can have an associated Alpha Feed — a curated list of LinkedIn/Twitter "sages" (thought leaders) and keywords whose posts are scraped daily and surfaced in a UI feed.

**Why:** Clients need to see what's resonating in their market before engaging. Rather than ad-hoc browsing, the alpha feed provides a curated, high-signal stream scoped to the ICP. Both LinkedIn (`alpha_feeds` table, `alpha-feed-collect-worker` task) and Twitter (`twitter_alpha_feeds` table, `twitter-alpha-feed-collect-worker` task) are supported from the same scheduler pattern. Spec generation (`alpha-feed-generate-spec`) uses Claude to propose sages and keywords automatically from the ICP definition, which a human then reviews and curates.
