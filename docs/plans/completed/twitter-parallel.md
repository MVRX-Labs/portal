...
**Status: IMPLEMENTED** (Phases 0–7 complete; Twitter Humaniser from Phase 1b was not built — LinkedIn Humaniser was also dropped.)

Key files:
- DB tables: `twitter_profiles`, `twitter_posts`, `twitter_post_snapshots`, `twitter_sync_runs`, `twitter_post_replies`, `twitter_post_engagements`, `twitter_alpha_feeds`
- Trigger tasks: `twitter-sync-profile`, `twitter-weekly-analytics`, `twitter-audit-generation`, `twitter-post-generator`, `twitter-to-linkedin`, `twitter-lead-upsert`, `twitter-engagement-slack-action`, `track-tweet`, `twitter-post-categoriser`, `twitter-alpha-feed-collect-worker`
- API schemas: `src/lib/api-schemas/twitter-profiles.ts`, `src/lib/api-schemas/twitter-engagement.ts`, `src/lib/api-schemas/twitter-alpha-feed.ts`

See `docs/design-decisions.md` → "Twitter/X as a Parallel Offering".
...
| **Post Humaniser**                       | `linkedin-humanizer`                                            | `tool_runs`                                                       | `/tools/linkedin-humanizer`                             | `POST /api/tools/linkedin-humanizer`                                              |
...
### Twitter/X Features (current)

- **Only**: `linkedin-to-twitter` conversion tool (throwaway, one-way)
- No profiles, no sync, no analytics, no engagement, no leads, no audit
