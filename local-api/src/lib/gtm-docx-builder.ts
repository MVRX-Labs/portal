...
import type { GTMStrategyContent } from "./gtm-schema.js";
...
function presenceAuditSection(pa: GTMStrategyContent["presenceAudit"]): (Paragraph | Table)[] {
  ...
  // Score table with colored score cells
  const cols = [25, 15, 60];
  out.push(
    new Table({
      width: { size: CONTENT_WIDTH, type: WidthType.DXA },
...
function channelDetailSection(
  ...
  // Page break before each channel detail
...
export async function buildGtmDocx(content: GTMStrategyContent): Promise<Buffer> {
  ...
  // Page break → Competitive Landscape
  children.push(new Paragraph({ children: [new PageBreak()] }));
  ...
  // Channel Details (each with its own page break)
  ...
  // Execution Roadmap (has its own page break)
