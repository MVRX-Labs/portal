...
## Apify for LinkedIn and Twitter Scraping

**Decision:** Use Apify actors for LinkedIn and Twitter/X profile data extraction.

**Why:** Neither platform has a public API suitable for scraping at this scale (LinkedIn has none; Twitter API v2 Basic is $100/mo for 10k reads — insufficient). Apify handles proxy rotation and anti-bot measures for both platforms. Configured via `APIFY_API_TOKEN`.
...
## Twitter/X: Separate Tables, Not Extended LinkedIn Tables

**Decision:** Twitter/X data lives in its own set of tables (`twitter_profiles`, `twitter_posts`, `twitter_post_snapshots`, `twitter_post_replies`, `twitter_post_engagements`, `twitter_sync_runs`) rather than extending the LinkedIn tables.

**Why:** The two platforms have genuinely different data shapes: tweet IDs vs Apify post IDs, retweets/quote tweets/bookmarks/views vs reposts/reactions, thread structure, character limits. Merging them into shared tables would require nullable columns for every platform-specific field. Separate tables keep queries simple, keep blast radius contained (a Twitter sync bug can't corrupt LinkedIn data), and mirror the platform separation in the codebase (`twitter-sync.ts`, `twitter-post-generator.ts`, etc.). The shared `leads` table is the exception — leads from either platform land there, identified by `linkedinUrl` and/or `twitterUrl`. Shared infrastructure (Apify client, Slack, humanisation libs, Claude agent runner) is reused across both platforms. See `docs/plans/active/twitter-parallel.md` for full context.
