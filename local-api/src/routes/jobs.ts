...
router.post("/gtm-strategy", (req, res) => {
  ...
  if (!runId || !companyName || !callbackUrl) {
    res.status(400).json({ error: "runId, companyName, and callbackUrl are required" });
    return;
  }

  log(runId, `Received gtm-strategy job for "${companyName}" (${industry})`);
  res.status(202).json({ status: "accepted" });
  ...
  runClaudeJob({
    ...
    postProcess: async (output) => {
      ...
      log(runId, `DOCX written → ${filepath} (${(buf.length / 1024).toFixed(0)} KB)`);
      return `GTM Strategy document saved: ${filename}`;
    },
  });
});
...
router.post("/sentiment-analysis", (req, res) => {
  ...
  runClaudeJob({
    ...
    postProcess: async (output) => {
      ...
      // Build a simple .docx from the JSON content
