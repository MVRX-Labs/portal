  ...
  if (error) {
    return (
      <div className="p-3 rounded-md bg-[rgba(239,68,68,0.1)] border border-[rgba(239,68,68,0.2)] text-sm text-(--destructive)">
        ...
        <p className="mt-1 text-(--muted)">Run ID: {dbRunId}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 rounded-md bg-[rgba(59,130,246,0.08)] border border-[rgba(59,130,246,0.2)] text-sm space-y-3">
      <div className="flex items-center justify-between">
        <p className="font-medium text-(--accent)">{progress?.step || "Starting..."}</p>
        <span className="text-xs text-(--muted) tabular-nums">{elapsed(createdAt)}</span>
      </div>
      ...
      {!progress && (
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-(--muted)">Waiting for task to start...</p>
        </div>
      )}
