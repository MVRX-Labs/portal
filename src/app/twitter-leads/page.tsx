...
import { getLeadsResponseSchema, getLeadCsvsResponseSchema } from "@/lib/api-schemas/leads";
...
function TwitterLeadsContent() {
  ...
  const navigate = (newPage: number) => {
    ...
    router.push(`/twitter-leads?${params}`);
  };

  if (!account) {
    return (
      <div>
        <h1 className="text-2xl font-bold mb-1">Twitter Leads</h1>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-bold">Twitter Leads</h1>
      </div>
      <p className="text-sm text-(--muted) mb-4">
        People who engaged with {account.name}&apos;s Twitter content
                  ...
                  leads.map((lead) => (
                    <tr key={lead.id} className="border-b border-(--border) last:border-0">
                      ...
                      <td className="px-3 py-1.5 text-(--muted)">
                        {lead.twitterHandle ? `@${lead.twitterHandle}` : "\u2014"}
                      </td>
...
export default function TwitterLeadsPage() {
  return (
