...
## Twitter/X Parallel Offering: Separate Schema, Shared Infrastructure

**Decision:** Build Twitter as a fully parallel offering to LinkedIn using separate database tables (`twitter_profiles`, `twitter_posts`, `twitter_post_snapshots`, etc.) rather than extending the LinkedIn tables.

**Why:** LinkedIn and Twitter have fundamentally different data shapes (tweet IDs vs Apify post IDs, retweets vs reposts, thread structure, impressions/views only on Twitter). Separate tables keep queries simple, avoid schema complexity on heavily-used LinkedIn tables, and contain blast radius per the decoupled systems principle. Infrastructure is shared: `apify-cache`, `leads`, `tool_runs`, `src/lib/claude-agent.ts`, `src/lib/slack.ts`, and `src/lib/ids.ts` all serve both platforms. Twitter uses Apify actors (same pattern as LinkedIn). See `docs/plans/completed/twitter-parallel.md`.

---

## Alpha Feed: Per-ICP Curated Content Stream

**Decision:** Alpha Feed is a per-ICP feature that discovers LinkedIn (and Twitter) thought leaders ("sages") and keywords via Claude, then scrapes their recent posts daily and stores them in `alpha_feeds` / `twitter_alpha_feeds` as a rolling 7-day window of top posts.

**Why:** Teams need a curated signal stream of what their target ICP is reading and sharing, without manually tracking hundreds of profiles. Two-phase approach: (1) `alpha-feed-generate-spec` uses Claude to discover relevant sages and keywords for an ICP; (2) `alpha-feed-collect-worker` runs daily to scrape and refresh the feed. Stored as JSONB (`dailyEntries`) on the feed record — no separate posts table needed since the data is ephemeral and display-only.
