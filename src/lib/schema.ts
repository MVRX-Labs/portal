...
export const accountActions = pgTable("account_actions", {
  ...
  knowledgeUnitId: text("knowledge_unit_id").references(() => knowledgeUnits.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const linkedinPostComments = pgTable(
  "linkedin_post_comments",
  {
    ...
    accountId: text("account_id")
      .notNull()
      .references(() => accounts.id),
    commentUrn: text("comment_urn").notNull(),
  ...
  (table) => ({
    uniquePostComment: unique().on(table.postId, table.commentUrn),
  })
);
