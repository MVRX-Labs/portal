/**
 * Post-processing step for Twitter audit reports.
...
export async function postProcessTwitterAudit(
  content: TwitterAuditContent,
  logger: Logger
): Promise<TwitterAuditContent> {
  const inputJson = JSON.stringify(content);
  logger.info(`Post-processing Twitter audit (${inputJson.length} chars input)`);
  ...
  const cleaned: TwitterAuditContent = JSON.parse(jsonStr);
