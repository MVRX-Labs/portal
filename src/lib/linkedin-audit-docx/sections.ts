...
import type { LinkedInAuditContent } from "../audit-schema";
import {
  tr,
  emptyPara,
  sectionHeading,
...
export function coverPage(c: LinkedInAuditContent): Paragraph[] {
  return [
    new Paragraph({
    ...
    new Paragraph({
      ...
      children: [tr("LinkedIn Audit", { bold: true, size: S.docTitle, color: C.darkNavy })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
...
export function executiveSummarySection(c: LinkedInAuditContent): (Paragraph | Table)[] {
...
export function scorecardSection(c: LinkedInAuditContent): (Paragraph | Table)[] {
  const out: (Paragraph | Table)[] = [];
  ...
  // Overall score as standalone right-aligned paragraph
  out.push(
    new Paragraph({
...
export function contentSections(c: LinkedInAuditContent): (Paragraph | Table)[] {
  const out: (Paragraph | Table)[] = [];
  sectionCounter = 3;
